...
description: >
  ...
  (aire-state.md) and audit trail (audit.md), then posts a structured review: inline
...
This project tracks work in `.spec/aire-docs/`. Use these to check the PR against **what was
actually planned**, not just the raw diff.

1. Read `.spec/aire-docs/aire-state.md` — find the **Story Tracker** rows in this PR's scope
...
2. Read `.spec/aire-docs/audit.md` — scan entries for the decisions and intent behind these
   changes so your review reflects the agreed plan.
3. If neither file has relevant entries, note that in your summary
   ("No aire-docs entries matched this PR — review derived from diff only") rather than
   inventing context.
...
4. **Never fabricate story/audit context** — if aire-docs has nothing for this PR, say so.
