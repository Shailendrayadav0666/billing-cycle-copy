...
Check whether `.spec/aire-docs/aire-state.md` exists and, if so, read `## Tracker` → `Type` from it — reuse that value silently, no question asked. If no such file/section exists (this skill runs standalone, before any aire project), ask once for THIS run only (nothing is persisted, per the guardrails below):
...
- **Tracker push only, no other writes.** The only write action is creating the Epic in the configured tracker (plus, if needed, one retry label/tag update per Step 3 of the hand-off sequence). Reading `## Tracker` from `.spec/aire-docs/aire-state.md` (Step -1) is read-only and does not violate this.
