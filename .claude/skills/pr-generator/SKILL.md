...
description: >
  ...
  Diffs the current branch against the latest target branch, reads .spec/aire-docs/aire-state.md
  (Story Tracker / story status) and .spec/aire-docs/audit.md (audit trail of what
...
- **Standalone mode** — invoked directly by the user via a trigger phrase ("raise a PR", "open a PR", etc.). **Read `## Branching` from `.spec/aire-docs/aire-state.md` FIRST** (silently — do not show it) to determine whether the target can be auto-resolved from the current branch, THEN decide whether to ask:
...
2. **Resolve the target branch (explicit input — never assume `main`)**:
   - **Invoked from a workflow** (dev-implement, requirements analysis, remediate, bug-fix-implement): use the target
     ...
     **Base Branch** for bug PRs (all recorded in
     `.spec/aire-docs/aire-state.md` under `## Branching`).
...
## Phase 3: AIRE Context — aire-state.md and audit.md

This project tracks work in two files under `.spec/aire-docs/`. Use them to ground the PR summary in
**what was actually planned and recorded**, not just the raw diff.

1. Read `.spec/aire-docs/aire-state.md`:
...
2. Read `.spec/aire-docs/audit.md`:
...
3. If either file doesn't exist or has no relevant entries, note that in the PR description
   ("No aire-state.md/audit.md entries found for this branch — summary derived from diff only")
...
**MANDATORY PREFIX — every PR title MUST start with `[EPIC]`, `[STORY]`, `[BUG]` or `[ENH]`** so reviewers can tell the PR type at a glance. Determine it from `## Branching` in `.spec/aire-docs/aire-state.md`:
...
## Summary
- [1-3 bullets on what changed and why, grounded in aire-state.md/audit.md context]

## Changes
- [Bulleted list of the main code changes, grouped by area]

## Audit Trail Context
[1-2 sentences summarizing relevant .spec/aire-docs/audit.md entries for this branch, or the
...
🔴 **EPIC CYCLES ONLY.** The archive auto-trigger applies **exclusively to an Epic → Base PR**. **Bug and Enhancement cycles archive MANUALLY** — on a `[BUG]` or `[ENH]` → Base PR, NEVER invoke `archive-epic`; print the manual reminder in Step 4 below instead. (Why: ve's `/ve-implement` test-plan PR and `ve-list-work` Option C amendments land on the bug/enhancement branch on their own schedule, so an archive taken at PR time silently omits `.spec/aire-docs/tests/...` or misses later test-plan edits. The operator archives once everything has landed — see `workflows/bug-fix-implement.md` Step 12 / `workflows/enhancement-implement.md` Step 19.)
...
3. The current branch is the recorded **Epic Branch** and the PR just opened targets the recorded **Base Branch** — read from `## Branching` in `.spec/aire-docs/aire-state.md`. (A story PR into the Epic branch does NOT match this condition.)
   - **If `## Branching` is missing** from `aire-state.md`, do NOT silently fail the check — ask the user:
     ```
     ⚠️ No ## Branching section found in aire-state.md, so I can't confirm this is the
...
2. **Never fabricate Story status or audit entries** — if `aire-state.md` or `audit.md`
