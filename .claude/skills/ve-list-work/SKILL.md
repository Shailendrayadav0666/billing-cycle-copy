...
description: >
  ...
  `audit.md`; only Option C writes to a story's `/ve-implement` test-plan files; Option A writes nothing.
when_to_use: >
...
of a cycle is **`stitch-delta`**; `ve-list-work` has no role there.

Why (bug/enhancement cycles changed to this): running sign-off post-merge on the base branch was
broken — by then `archive-epic`'s workspace reset had already deleted `.spec/aire-docs/aire-state.md`,
...
> (`.spec/aire-docs/tests/<TICKET-ID>-<title>/`) and never changes status. This skill changes
...
- **Option B**: updates the `## Story Tracker` in `.spec/aire-docs/aire-state.md` and appends one
  entry to `.spec/aire-docs/audit.md` recording every approval and rejection. On JIRA/ADO/GITHUB it also adds a
  comment and an `ve-approved` / `ve-rejected` label/tag per item; LOCAL updates only the Story Tracker.
- **Option C**: edits a story's `/ve-implement` test-plan artifacts under `.spec/aire-docs/tests/` — the only
...
2. `.spec/aire-docs/aire-state.md` must exist with a `## Story Tracker` and a `## Branching` section.
...
1. Read `## Branching` and `Workflow Type` in `aire-state.md` to resolve the **integration
...
Read `## Story Tracker` in `.spec/aire-docs/aire-state.md`. It carries the columns the development
workflows write:
...
- 🔴 **Bug / enhancement cycles — do NOT require the `[BUG]`/`[ENH]` PR to be merged.** That PR targets
  ...
  2. the ve's own test-plan PR (`ve/<TICKET-ID>-…` → the cycle branch) has **merged**, so
     `.spec/aire-docs/tests/<TICKET-ID>-…/` exists in the working tree.
...
`aire-state.md` per `requirements-analysis.md` Step 5.1's canonical table format
...
`.spec/aire-docs/tests/<TICKET-ID>-<title>/automation-summary.md`. If either exists, show ✅ (with the
...
|---|-------|------------|-------|----|-----------|-------------------|------------|-------------|
| 1 | 1.1 | PROJ-101 | [title] | #12 | [date] | `.spec/aire-docs/tests/PROJ-101-<title>/` ✅ | ✅ 13 passing, 1 fixme | 🔵 In Development |
...
1. **Story Tracker** (`.spec/aire-docs/aire-state.md` → `## Story Tracker`):
...
2. **Tracker** — only if the story's `Tracker ID` column holds a non-LOCAL value (not `—`). Dispatch per `## Tracker` → `Type`, per `common/tracker-sync.md` Section 4/Section 9/Section 10, doing all three (comment, label/tag, transition), in this order:
   - **a) Comment (MANDATORY)** — leading with the exact phrase
     **`ve approved the story`**: JIRA `addCommentToJiraIssue`, ADO `az boards work-item update --discussion`, GITHUB `gh issue comment`:
     ...
     Merged PR: <PR URL>
     Test steps: .spec/aire-docs/tests/<TICKET-ID>-<title>/
...
2. **Tracker** — only if the story's `Tracker ID` column holds a non-LOCAL value (not `—`), dispatched per `## Tracker` → `Type`:
   ...
   - LOCAL stories (`Tracker ID = —`/`LOCAL`): record the rejection in `audit.md` only — no external comment or
     label to apply.
...
Resolve the **Parent Epic** from `## Tracker` in `aire-state.md`:
...
edits the `/ve-implement` skill's output (`.spec/aire-docs/tests/<TICKET-ID>-<title>/`) — it never touches
...
1. **Ask which story's test plan to change** (skip if already given):
   ...
   [Answer]:
   ```
2. **Resolve the test folder**: `.spec/aire-docs/tests/<TICKET-ID>-<title-kebab>/` (or
   `.spec/aire-docs/tests/story-<N.M>-<title-kebab>/` for a local-only story — see
   `.aire-rule-details/implementation/test-plan.md` Output Location for the exact naming
...
5. **Apply the change** using the exact rules and format from
   `.aire-rule-details/implementation/test-plan.md`:
...
7. On confirmation, write the file(s) and announce:
   `✅ Updated .spec/aire-docs/tests/<TICKET-ID>-<title>/<file> — added TC-[PLAN]-[nn], traced to AC-[n].`
...
**File changed**: .spec/aire-docs/tests/<TICKET-ID>-<title>/<file>
...
➡️ NEXT ACTIONS:
   ...
   2️⃣  Execute the manual test steps in `.spec/aire-docs/tests/<TICKET-ID>-<title>/`
...
- 🔴 **Status is read LIVE from the tracker (Step 3) for JIRA/ADO/GITHUB, never trusted from `aire-state.md`
...
- 🔴 **Runs NO build and NO tests.** The ve tests the merged work manually, using the steps `/ve-implement`
  generated in `.spec/aire-docs/tests/<TICKET-ID>-<title>/`.
- 🔴 **Option C only edits `/ve-implement` test-plan artifacts** (Step 8) — new/changed test cases MUST
  ...
  `.aire-rule-details/implementation/test-plan.md`. It never changes story or tracker status.
...
- 🔴 **Close with the handoff that matches the cycle**: epic → `pr-generator` for the Epic PR;
  ...
  bug/enhancement cycle, and never send the ve to the base branch for anything but `stitch-delta`.
