...
is configured (`## Tracker` → `Type` in `.spec/aire-docs/aire-state.md`). **Do NOT ask for anything beyond these fields** — no preconditions, steps to reproduce,
...
1. Read `## Tracker` → `Type` from `.spec/aire-docs/aire-state.md`. If it doesn't exist yet, ask the Tracker Selection question (`common/tracker-sync.md` Section 1) first.
...
3. Determine the target **project/repo** (skip for LOCAL):
   ...
   - Else check `.spec/aire-docs/aire-state.md` (`## Tracker` → `Project Key / Repo / Org`) / `.spec/aire-docs/audit.md`
...
**LOCAL**:
- No external issue. Mint a local ID `BUG-LOCAL-N` (next unused N found by scanning `aire-state.md`) and record the full description body as a new row/entry in the local Story Tracker (or a local defects doc if the project keeps one) — the local file IS the ticket.
...
2. Build the full issue URL where applicable: `<site-base-url>/browse/<ISSUE-KEY>` for JIRA (site base
   URL from `getAccessibleAtlassianResources`, or reuse the base already recorded in `.spec/aire-docs/`), the
   work-item/issue URL directly for ADO/GITHUB. LOCAL has no URL — use the local ID.
3. **Log in `.spec/aire-docs/audit.md`** (append-only at the end — never rewrite the file):
   ```markdown
   ## Defect Raised (ve)
...
7. **Always log the raised defect in `.spec/aire-docs/audit.md`** with the full tracker hyperlink (or local ID).
