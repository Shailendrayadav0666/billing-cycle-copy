...
0. **Resolve** which tracker is configured (read `## Tracker` from `.spec/aire-docs/aire-state.md` if it exists; otherwise ask once for this run).
...
If `## Tracker` isn't already recorded in `.spec/aire-docs/aire-state.md`, ask once which tracker to use (JIRA / ADO / GITHUB / Local) before continuing. Then ask the user, per that tracker:
```
...
- **No file or directory creation.** Do not create, write, or modify any file or folder in the workspace (reading `## Tracker` from `.spec/aire-docs/aire-state.md`, if it exists, is fine — that's a read, not a write). The refined intent lives in the configured tracker only (or in chat, for Local — never a local file).
