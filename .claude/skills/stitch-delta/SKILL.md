...
- Ledger format: `| Cycle | Tracker Item | Delta Folder | Stitched At | Commit Range | Documents Updated |` — `Cycle` is `epic`, `bug`, or `enhancement`, `Tracker Item` is the cycle's tracker ID (JIRA/ADO/GITHUB/LOCAL). If an existing ledger uses the old format (no Cycle/Tracker Item columns), migrate it in place first per the agent file's Step 2 migration rule
- Stitching MUST follow the "Delta Reverse Engineering & Stitching" section of `.aire-rule-details/planning/reverse-engineering.md`, including the post-stitch verification gate
- Push is confirm-first; on a race-rejected push, pull --rebase, re-check the ledger, retry — NEVER force-push
- **If the direct push is BLOCKED by branch protection** (base rejects direct pushes / requires a PR): fall back to raising a **PR** into the base branch from a `docs/stitch-delta-*` branch. That PR MUST carry the `ai-generated` and `aire-v1.0` labels (version hardcoded to `1.0`). See the agent file's Step 5.4.
