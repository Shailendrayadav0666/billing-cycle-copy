...
- 🔴 **Do NOT pre-load** `extensions/testing/playwright-automation/playwright-automation.md` or `implementation/test-plan.md` — Step 6.7 reaches them by **invoking the `ve-implement` and `playwright-implement` skills in WORKFLOW MODE**, and each skill loads its own rule files.
  ...
  1. **The ONLY permitted producer of `tests/e2e/**` and `tests/playwright-specs/**` is the `playwright-implement` skill's own Planner/Generator subagents.** If you are about to `Write`/`Edit` a `.spec.*` file and no `Skill(playwright-implement)` call has happened this run, you are violating the gate.
...
1.7. **🔴 EXECUTION-CONTEXT CHECK (MANDATORY — on EVERY invocation, before Story Selection)**: Step 6.7's Playwright gate invokes Playwright's own Planner/Generator/Healer via the **Agent tool**. **A fork, or any subagent instructed "do NOT spawn subagents / you ARE the fork, execute directly", cannot call it** — the gate is then structurally impossible, no matter what is installed. Determine now whether this run can call the Agent tool:
   - **Can** → continue silently.
   - **Cannot** → announce it immediately: ` This run cannot call the Agent tool, so the Playwright UI gate (Step 6.7) cannot run here. A backend-only story will complete normally; a UI story will HALT at 6.7 — run that one from a normal session instead.` Record it in `runtime-artifacts/audit.md` and continue.
   🔴 **It is NEVER a reason to hand-author Playwright specs** (`implementation/code-generation.md` Step 11d, Part B catch-all) — a UI story halts and is re-run from a context that can invoke subagents. Observed in a real run: `dev-implement` inside a fork silently hand-authored five `.spec.ts` files instead of halting, producing a "passing" gate that never exercised a browser.
...
6.7. ** TEST PLANS + PLAYWRIGHT UI AUTOMATION GATE (MANDATORY — after the Static Eval Gate, before Code Review, same run)**: execute **Step 11d** of `code-generation.md`. 🔴 **It INVOKES two existing skills via the Skill tool, in WORKFLOW MODE — it never re-implements them**, and 🔴 **the two halves have DIFFERENT applicability**: part 1 runs for **every** story; part 2 only when the story's plan includes a Frontend Components Generation step (plan-derived and automatic — never asked).
   1. **`ve-implement` — 🔴 ALWAYS, every story** (skipped only if `spec/test-plans/<TICKET-ID>-<title>/` already has this story's manual steps from ve's own run, in any state) — invoked with this story and `mode: workflow`, which per `agents/ve-implement-agent.md` **Mode Detection** skips its story-picker, its `ve/…` branch, its Approve/Request-Changes checkpoint and its push/PR; the generated `spec/test-plans/…` files stay in the working tree and ride THIS story's commit. **This is what lets the ve simply execute the plan after the story PR merges, instead of having to generate it first.**
...
- 🔴 **ONE SANCTIONED PAUSE — a Playwright agent install (Step 6.7).** If the Playwright UI Automation Gate had to run `npx playwright init-agents --loop=claude`, it **HALTS immediately and asks the user to restart the Claude Code session**, because MCP servers load at session start and the `playwright-test` server it just registered is not connected in this session — the Planner/Generator/Healer physically cannot run. 🔴 **This is a hard stop, not an approval prompt** — the same class as a Retry-Limit halt — so it does NOT violate the no-prompts rule below. Nothing is committed, pushed or transitioned; on restart the user re-invokes `dev-implement` with the same story and it resumes **at Step 6.7**, skipping every gate already logged as passed. No install needed → no pause.
- 🔴 **THE RUN HAS NO PROMPTS AFTER THE STORY KEY.** Story selection → branch → baseline (regression + static eval) → plan → code → coverage → test placement check → regression → static eval gate → review → auto-remediate → [manifest reconciliation → commit → **CI preflight (SH-LOOP-9)**] → push → PR (pr-generator **workflow mode, Phase 5 skipped**) → labels → Story Tracker PR/Merged update → [CI attestation] → auto `pr-review` → Section F handoff, all uninterrupted. The bracketed CI-specific steps run only when `## CI/CD Configuration` `Enabled: Yes` (Step 1.2 opt-in) — when `Enabled: No` they are silently skipped, never asked about again, and the chain is simply: review → auto-remediate → commit → push → PR → auto `pr-review` → Section F handoff. Asking anything in that chain (approve the plan, approve the review, whether to push, whether to open the PR, whether the title/body/labels are OK, or whether to run CI) is a defect. Announce each action; never ask mid-run.
