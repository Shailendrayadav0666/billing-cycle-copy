...
> **Scope pulled**: document_id 3157 (artifact_type `epic`, v1, CURRENT) — the Epic named in the user's request
> **Fetched**: 2026-08-31T11:05Z
> **Freshness**: Atlas reports updated_at 2026-08-25T11:29:10Z
>
> **PATH MAPPING NOTE**: Atlas documents the upstream repo layout (`backend/`, `frontend/`).
> In THIS workspace the backend lives at `src/backend/` and the frontend at `frontend/`.
> Every `backend/main.py` reference below means `src/backend/main.py` here.
> Frontend paths are unchanged. Atlas content itself is reproduced verbatim.
...
**As a Standard subscriber clicking "Upgrade to Premium", I want to see a confirmation step showing the exact prorated charge before I commit, so I am not surprised by the amount.**
...
- A success banner is shown: "You are now on Premium! $10.00 was charged."
...
**As a Premium subscriber visiting the Billing page, I should not see the upgrade button, so I am not confused into attempting a redundant upgrade.**
