...
2. **`architecture.md` + Architecture Rubric** (ALWAYS — at the STOP CHECKPOINT, automatic, no gate):
   ...
   repo's real stack/thresholds; when the user opts out, no CI file is generated and the cycle's
   `dev-implement`/`bug-fix-implement`/`enhancement-implement` runs go straight from the local gates +
   automated code review to the PR raise and automated PR review.
...
1.2. **MANDATORY: CI/CD Setup Opt-In (ASK-FIRST, CHAT-ONLY — same pattern as Tracker Selection)**:
   before loading `common/ci-pipeline-generation.md` or generating any CI/CD file, ask the user
   whether to set up the CI/CD eval pipeline at all. **Skip and reuse if `## CI/CD Configuration`
   already exists** in `runtime-artifacts/aire-state.md` (resumed project) — never re-ask.
   - **First check whether `.github/workflows/agentic-eval-pipeline.yml` already exists** (inherited
     from base, or present on this branch). If it does, no need to ask — record
     `## CI/CD Configuration` `Enabled: Yes` `Source: pre-existing` automatically and proceed to Step 1.6.
   - **Otherwise, ask** (conversational, persist only the answer — no question `.md` file):
     ```
     Set up the CI/CD eval pipeline for this project? (yes/no)
     ```
   - **yes** → record `## CI/CD Configuration` `Enabled: Yes` `Source: user opt-in` in
     `runtime-artifacts/aire-state.md`, log the question + raw answer in `runtime-artifacts/audit.md`,
     proceed to Step 1.6.
   - **no** → record `## CI/CD Configuration` `Enabled: No` `Source: user opt-out` in
     `runtime-artifacts/aire-state.md`, log the question + raw answer in `runtime-artifacts/audit.md`,
     **SKIP Step 1.6 and Step 4 entirely** — never load `common/ci-pipeline-generation.md`, never
     generate `.github/workflows/agentic-eval-pipeline.yml`, `sonar-project.properties`, or any
     `tests/.evals/scripts/*` CI script — and go straight to Step 2.
...
1.6. **Generate the CI pipeline** (CONDITIONAL on `## CI/CD Configuration` `Enabled: Yes` — Step 1.2;
   automatic, one setup gate when it runs), per `common/ci-pipeline-generation.md`:
...
3. **Commit + push the design artifacts on the Epic branch (automatic — unblocks ve)**: stage
   ...
   created by Step 1.5's create-if-missing rule; PLUS the CI files from Step 1.6 when it ran),
   ...
   branch is on origin**.
4. **🧪 Epic-level pre-handoff smoke test** (CONDITIONAL on `## CI/CD Configuration` `Enabled: Yes` —
   there is no CI pipeline to smoke-test when the user opted out at Step 1.2, so this step is SKIPPED
   entirely, announced, when `Enabled: No`; automatic, HARD HALT on exhaustion when it runs), per
...
- Per project, CONDITIONAL on the `## CI/CD Configuration` opt-in: **`.github/workflows/agentic-eval-pipeline.yml`** (`common/ci-pipeline-generation.md`) — absent entirely when the user opted out
