...
> **Estate**: Billing-Cycle-Helix-Workshop (solution_id 951) — repo `Shailendrayadav0666/Billing-Cycle`, branch `main`, last ingested commit `bcec649e08f2dbec435c24066deae6a1d6d71192`
> **Scope pulled**: Whole-estate deep dive (single document covers the entire repo — no sub-scoping required)
> **Fetched**: 2026-09-17T11:47:27Z
> **Freshness**: Document version 17, last updated 2026-09-16T10:25:25.542674+00:00 (Atlas's own `updated_at`)
...
```mermaid
graph LR
    mainjsx["main.jsx"] -->|imports| Appjsx["App.jsx"]
    Appjsx -->|imports| AuthContextjsx["AuthContext.jsx"]
    Appjsx -->|imports| Loginjsx["Login.jsx"]
    Appjsx -->|imports| Billingjsx["Billing.jsx"]
    Appjsx -->|imports| Tasksjsx["Tasks.jsx"]
    Loginjsx -->|imports| AuthContextjsx
    Billingjsx -->|imports| AuthContextjsx
    Tasksjsx -->|imports| AuthContextjsx
    mainpy["main.py"] -.->|no internal imports\nstandalone monolith| mainpy
```
...
graph TD
    ...
    Billingjsx -->|imports| AuthContextjsx
    Tasksjsx -->|imports| AuthContextjsx
    mainjsx -->|imports| indexcss["index.css"]
    Appjsx -->|imports| Appcss["App.css"]
    Loginjsx -->|imports| Appcss
    Billingjsx -->|imports| Appcss
    Tasksjsx -->|imports| Appcss

    subgraph Backend
        mainpy["main.py"] -->|imports| fastapi["fastapi"]
        mainpy -->|imports| pydantic["pydantic"]
        mainpy -->|imports| uvicorn["uvicorn"]
        mainpy -->|imports| pymultipart["python-multipart"]
        mainpy -->|imports| stdlib["datetime / pathlib"]
    end
```
...
| `App.jsx` | 1 | 5 | 0.83 | Unstable — depends on most modules |
| `Billing.jsx` | 1 | 2 | 0.67 | Medium |
| `Login.jsx` | 1 | 2 | 0.67 | Medium |
| `Tasks.jsx` | 1 | 2 | 0.67 | Medium |
| `main.py` | 0 | 6 | 1.00 | Backend entry point — maximally unstable by design; no internal consumers |
...
|---|---|---|---|---|
| fastapi | unpinned | REST API framework | No | Pin version immediately |
| uvicorn[standard] | unpinned | ASGI server | No | Pin version immediately |
| python-multipart | unpinned | Form / multipart parsing | No | Pin version immediately |
...
|---|---|---|---|
| `backend/main.py` | 8 | 213 | Highest — all logic in one file |
| `AuthContext.jsx` | 5 | 67 | High for its size |
...
| `Billing` | `Billing.jsx` | 100–181 (81 LOC) | Long — renders 4 sub-sections inline |
| `register` | `main.py` | 115–173 (58 LOC) | Long — creates 3 dicts of mock data inline |
| `AuthProvider` | `AuthContext.jsx` | 6–64 (58 LOC) | Long — useEffect + login + register + logout all in one |
...
| Medium | Duplicated fetch pattern (3×) | `Billing.jsx`, `Tasks.jsx`, `AuthContext.jsx` | Low | Extract custom hooks |
| Medium | 4 sub-components inline in `Billing.jsx` | `Billing.jsx:5–98` | Low | Reusability blocked |
| Medium | `register()` 58 LOC mock data factory | `main.py:115–173` | Low | Extract factory function |
| Medium | Unpinned Python dependencies | `requirements.txt` | Very Low | Fresh installs may break |
...
| `App.jsx` | 0% | High — route guards, protected route logic |
...
|---|---|---|
| Proration calculation `(50-20)/30 x days_remaining` | Critical | Math error = wrong charge; no safety net before PRD upgrade ships |
...
| `register()` duplicate email check | High | Creates duplicate accounts silently if guard fails |
| `add_task` ID generation under concurrency | High | Race condition on `max(id)` could produce duplicate IDs |
| Billing data null/missing response handling | High | `Billing.jsx` hangs on 401 with no timeout or error UI |
| Plan badge rendering (`Standard` vs `Premium`) | Medium | PRD feature depends on this — no regression protection |
...
> **Disclaimer:** This is NOT a security audit. This is a pattern-based review flagging areas for professional security assessment.
...
| **Token storage** | `localStorage` — susceptible to XSS theft | High |
| **Password hashing** | Passwords stored and compared in plaintext | Critical |
| **Brute force protection** | None — no rate limiting on `/api/auth/login` | High |

### Authorization

| Aspect | Finding | Severity |
|---|---|---|
| **Frontend guard** | `ProtectedRoute` checks for token presence only — any non-empty string passes | High |
...
| **Email format validation** | No email format check on register — any string accepted as email | Medium |
| **Frontend validation** | No client-side validation on Login form beyond HTML `type="email"` | Medium |
...
| Aspect | Finding | Severity |
|---|---|---|
| **Hardcoded credentials** | Demo account `tpg@example.com` / `password` hardcoded in `main.py:19–29` | Medium (POC) |
| **Environment variables** | No `.env` usage — no config externalization | Medium |
| **CORS config** | `allow_origins=["*"]` — accepts requests from any origin | Medium |
...
| **Concurrent write race** | `tasks_data` ID generation: `max(id) + 1` | Race condition under concurrent POSTs — duplicate IDs possible |
...
3. **Restrict CORS origins** — Change `allow_origins=["*"]` to explicit list of allowed origins before any non-local deployment
4. **Add rate limiting** — `slowapi` middleware on `/api/auth/login` and `/api/auth/register`
5. **Move hardcoded demo credentials to environment variables**
...
| **Frontend state** | Component-level `useState` only — lost on navigation | React Query or SWR would add stale-while-revalidate caching with minimal effort |
...
| **Proration formula rationale** | `(50-20)/30 x days_remaining` — no docs explain the pricing assumptions (30-day month, $20->$50 delta) | Medium — PRD specifies it but no ADR |
...
| **Mock data structure** | `billing_data`, `users`, `tasks_data` dicts have implicit schemas with no documentation | Medium — any new contributor must reverse-engineer from code |
...
| **No production deployment guide** | README documents local setup only — no Docker, no cloud deployment, no env var guide | Medium |
...
| 3 | **Pin Python dependency versions** — add specific versions to `requirements.txt` for `fastapi`, `uvicorn`, `python-multipart` | Step 5 Dependencies | High | 15 min |
| 4 | **Document the auth model** — add inline comments + a short `ARCHITECTURE.md` explaining email-as-token, in-memory store, and planned production path | Step 10 Docs | High | 1 hr |
